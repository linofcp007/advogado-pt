...
**EN:** Activate the `advogado-pt` skill and read the reference for the area named in $ARGUMENTS using the `ler_referencia` MCP tool (`nome`: e.g. `laboral`, `cobrancas`, `rgpd`, `imobiliario`, `valores-2026`). If no MCP is available, read the file in `references/`. If the area does not exist, list the available references. Summarise what matters for the case and cite statutes/articles precisely.
